    ...
    <div class="row">
                    ...
                    <div class="pull-right box-tools">
                        ...
                        <?php if($permiso['add']) { ?>
                            ...
                            <?php echo $this->Html->link('<i class="fa fa-plus" ></i>',
                                        ['controller' => 'OutputMicro', 'action' => 'add','id'=>$id],
        ...
        <thead class="headTablaListado">
            <tr class="text-uppercase text-center th-head-inputs">
                 ...
                 <th style="min-width: 150px;">NÚMERO DE TUBOS</th>
        ...
        <tbody>
            <?php $item = 1; ?>
            <?php foreach ($outputMicro as $outputMicro): ?>
             <tr>
                <td><?php echo $this->Number->format($item) ?></td>
                <td><?php echo ($outputMicro->reqcode) ?></td>
                <td><?php echo ($outputMicro->reqname) ?></td>
                <td><?php echo  $outputMicro->exitdate=null || $outputMicro->exitdate==''?'':date("d-m-Y", strtotime($outputMicro->exitdate))?></td>
                <td><?php echo ($outputMicro->numtubexit) ?></td>
                <td><?php echo ($outputMicro->object) ?></td>

                <td>

                      <?php if($permiso['view']) { ?>
                       <a href="<?php echo $this->Url->build('/'.$this->request->url.'/ver/'.$outputMicro->id, true) ?>" class="btn btn-info btn-xs" data-toggle = "tooltip" title = "Ver información de la Salida de Material."><i class="fa  fa-search-plus"></i></a>
                       ...
                       <?php
                        ...
                        <?php if($permiso['edit'] /*&& $validar*/ ) { ?>
                          <a href="<?php echo $this->Url->build('/'.$this->request->url.'/editar/'.$outputMicro->id, true) ?>" class="btn btn-primary btn-xs" data-toggle = "tooltip" title = "Editar información de la Salida de Material."><i class="fa fa-pencil-square-o"></i></a>
                          <?php } ?>

                        <?php if($permiso['delete'] /*&& $validar*/ ) { ?>
                            <?php echo $this->Html->link('<i class="fa fa-trash-o"></i>', "#",
                                                ['class' => 'btn btn-danger btn-xs delete-btn', 'escape' => false, "data-id"=>$outputMicro->id, 'data-toggle' => "tooltip", 'title' => "Eliminar la Salida de Material."])
                       ?>
                       <?php } ?>
