                ...
                <div class="box-header with-border">
                    ...
                    <div class="pull-right box-tools">
                        ...
                        <?php echo $this->Html->link('<i class="fa fa-plus" ></i>',
                                    ['controller' => 'OutputField', 'action' => 'add','id'=>$id],
                                ...
                                <table id="tablaListado" class="table table-striped table-bordered table-hover">
                                    ...
                                    <thead class="headTablaListado">
                                        ...
                                        <tr class="text-uppercase text-center th-head-inputs">
                                             ...
                                             <th style="min-width: 150px;">CANTIDAD DE MUESTRA</th>
                                             <th style="min-width: 150px;">DESTINO DE LA MUESTRA</th>
                                    ...
                                    <tbody>
                                        ...
                                        <?php $item = 1; ?>
                                        <?php foreach ($outputField as $outputField): ?>
                                         <tr>
                                            <td><?php echo $this->Number->format($item) ?></td>
                                            <td><?= h($outputField->reqcode) ?></td>
                                            <td><?= h($outputField->reqname) ?></td>
                                            <td><?= empty($outputField->exitdate)? '' : date("d-m-Y", strtotime($outputField->exitdate))  ?></td>
                                            <td><?= h($outputField->samplamount) ?></td>
                                            <td><?= h($outputField->destiny) ?></td>

                                            <td>

                                                <?php if($permiso['view']) { ?>

                                                   <a href="<?php echo $this->Url->build('/'.$this->request->url.'/ver/'.$outputField->id, true) ?>" class="btn btn-info btn-xs" data-toggle = "tooltip" title = "Ver información de la Salida de Material."><i class="fa  fa-search-plus"></i></a>

                                                <?php } ?>

                                                <?php if($permiso['edit']) { ?>

                                                  <a href="<?php echo $this->Url->build('/'.$this->request->url.'/editar/'.$outputField->id, true) ?>" class="btn btn-primary btn-xs" data-toggle = "tooltip" title = "Editar información de la Salida de Material."><i class="fa fa-pencil-square-o"></i></a>
                                                ...
                                                <?php if($permiso['delete']) { ?>
                                                  ...
                                                  <?php echo $this->Html->link('<i class="fa fa-trash-o"></i>', "#",
                                                                            ['class' => 'btn btn-danger btn-xs delete-btn', 'escape' => false, "data-id"=>$outputField->id, 'data-toggle' => "tooltip", 'title' => "Eliminar la Salida de Material."])
                                                   ?>
