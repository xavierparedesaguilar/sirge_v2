    ...
    <div class="row">
                ...
                <div class="box-header with-border">
                    ...
                    <div class="pull-right box-tools">
                        ...
                        <?php echo $this->Html->link('<i class="fa fa-plus" ></i>',
                                    ['controller' => 'longTermConservationMicro', 'action' => 'add','id'=>$id],
                                    ['class' => 'btn btn-success', 'data-toggle'=> "tooltip",  'title'=> "Conservación a Largo Plazo", 'escape'=>false] )
                        ?>
                        <?php } ?>
        ...
        <thead class="headTablaListado">
            <tr class="text-uppercase text-center th-head-inputs">
                 ...
                 <th style="min-width: 220px;">NOMBRE DEL RESPONSABLE</th>
        ...
        <tbody>
            <?php $item = 1; ?>
            <?php foreach ($longTermConservationMicro as $longTermConservationMicro): ?>
             <tr>
                <td><?php echo $this->Number->format($item) ?></td>
                <td><?php echo ($longTermConservationMicro->constime == NULL || $longTermConservationMicro->constime=='') ?'':date("d-m-Y", strtotime($longTermConservationMicro->constime))  ?></td>
                <td><?php echo ($longTermConservationMicro->consresponsible) ?></td>
                <td><?php echo ($longTermConservationMicro->consrem) ?></td>
                <td><?php echo $longTermConservationMicro->mediolargo==NULL?'': $longTermConservationMicro->mediolargo->name ?></td>
                <td><?php echo ($longTermConservationMicro->renewal_date == NULL || $longTermConservationMicro->renewal_date=='') ?'':date("d-m-Y", strtotime($longTermConservationMicro->renewal_date))  ?></td>
                <td><?php echo ($longTermConservationMicro->longtermtemp) ?></td>

                <td>

                      <?php if($permiso['view']) { ?>
                       <a href="<?php echo $this->Url->build('/'.$this->request->url.'/ver/'.$longTermConservationMicro->id, true) ?>" class="btn btn-info btn-xs" data-toggle = "tooltip" title = "Ver Información Conservación a Largo Plazo."><i class="fa  fa-search-plus"></i></a>
                       <?php } ?>
                      ...
                      <a href="<?php echo $this->Url->build('/'.$this->request->url.'/editar/'.$longTermConservationMicro->id, true) ?>" class="btn btn-primary btn-xs" data-toggle = "tooltip" title = "Editar Información Conservación a Largo Plazo."><i class="fa fa-pencil-square-o"></i></a>
                      <?php } ?>

                        <?php if($permiso['delete'] /*&& $validar*/ ) { ?>
                      <?php echo $this->Html->link('<i class="fa fa-trash-o"></i>', "#",
                                                ['class' => 'btn btn-danger btn-xs delete-btn', 'escape' => false, "data-id"=>$longTermConservationMicro->id, 'data-toggle' => "tooltip", 'title' => "Eliminar  Conservación a Largo Plazo."])
                       ?>
                       <?php } ?>
