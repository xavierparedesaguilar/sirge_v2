    ...
    <div class="row">
                    ...
                    <div class="pull-right box-tools">
                        ...
                        <?php if($permiso['add']) { ?>
                            ...
                            <?php echo $this->Html->link('<i class="fa fa-plus" ></i>',
                                        ['controller' => 'InputField', 'action' => 'add','id'=>$id],
        ...
        <thead class="headTablaListado">
            <tr class="text-uppercase text-center th-head-inputs">
                 ...
                 <th style="min-width: 165px;">COD. DE ACCESIÓN DEL DONANTE</th>

                 <th style="min-width: 160px;">FECHA DE ENTRADA</th>
                 <th style="min-width: 170px;">NÚMERO DE MUESTRAS</th>
        ...
        <tbody>
            <?php $item = 1; ?>
            <?php foreach ($inputField as $inputField): ?>
             <tr>
                <td><?php echo $this->Number->format($item) ?></td>
                <td><?= h($inputField->donorcore) ?></td>
                <td><?= h($inputField->donorname) ?></td>
                <td><?= h($inputField->donornumb) ?></td>
                <td><?= empty($inputField->enterdate)? '' : date("d-m-Y", strtotime($inputField->enterdate))  ?></td>
                <td><?= h($inputField->numtubent) ?></td>
                ...
                <td>
                    ...
                    <?php if($permiso['view']) {  ?>
                      ...
                      <a href="<?php echo $this->Url->build('/'.$this->request->url.'/ver/'.$inputField->id, true) ?>" class="btn btn-info btn-xs" data-toggle = "tooltip" title = "Ver información de la Entrada de Material."><i class="fa  fa-search-plus"></i></a>

                    <?php } ?>

                    <?php if($permiso['edit']) {  ?>

                        <a href="<?php echo $this->Url->build('/'.$this->request->url.'/editar/'.$inputField->id, true) ?>" class="btn btn-primary btn-xs" data-toggle = "tooltip" title = "Editar información de la Entrada de Material."><i class="fa fa-pencil-square-o"></i></a>
                    ...
                    <?php if($permiso['delete']) {  ?>
                      ...
                      <?php echo $this->Html->link('<i class="fa fa-trash-o"></i>', "#",
                                                ['class' => 'btn btn-danger btn-xs delete-btn', 'escape' => false, "data-id"=>$inputField->id, 'data-toggle' => "tooltip", 'title' => "Eliminar la Entrada de Material."])
                       ?>
