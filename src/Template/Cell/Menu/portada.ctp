...
<?php if($total_1 > 0){ ?>
    <div class="col-lg-4 col-md-12 col-xs-12">
        <fieldset>
            <legend><i class="fa fa-toggle-on" aria-hidden="true"></i> RECURSOS GENÉTICOS</legend>
                    ...
                    switch ($modulo->resource_id) {
                        case 4:
                            $modulo_title = $modulo->title;
                             if($modulo->id == 1){
                                 $modulo->icon = 'icons/utilitarios.png';
                              }else if($modulo->id == 47){
                                 $modulo->icon = 'icons/user.png'; 
                              }else if($modulo->id == 42){
                                $modulo->icon = 'icons/publicacion.png'; 
                              }else if($modulo->id == 45){
                                $modulo->icon = 'icons/ordenes.png'; 
                              }else if($modulo->id == 46){
                                $modulo->icon = 'icons/pagos.png'; 
                              }else if($modulo->id == 17){
                                $modulo->icon = 'icons/insitu.png'; 
                              }
                            break;
                        case 3:
                        ...
                        <a href="<?php echo $this->Url->build('/'.$this->request->url . '/' . $modulo->slug, true) ?>" class="small-box-footer">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    <span class="info-box-number"><h4><strong><?php echo $modulo_title ?></strong></h4></span>
                                </div>
                            </div>
                        </a>
                    </div>
...
<?php if($total_2 > 0){ ?>
    <div class="col-lg-4 col-md-12 col-xs-12">
        <fieldset>
            <legend><i class="fa fa-toggle-on" aria-hidden="true"></i> SERVICIOS</legend>
...
<?php if($total_3 > 0){ ?>
    <div class="col-lg-4 col-md-12 col-xs-12">
        <fieldset>
            <legend><i class="fa fa-cogs" aria-hidden="true"></i> CONFIGURACIÓN</legend>
            <?php foreach ($modulos as $key => $modulo): ?>
                <?php $modulo_title = $modulo->title; ?>
