...
class AjaxController extends AppController
{
    ...
    public function detalleespecie($id = null)
    {
        if ($this->request->is('ajax')) {
            ...
            $concat = $detalle->func()->concat(['genus' => 'identifier', ' ', 'species' => 'identifier' , ' ', 'autor' => 'identifier']);
    ...
    public function getnombrecomun($id = null)
    {
        if ($this->request->is('ajax')) {
            ...
            $detalle = $this->Specie->find()->select(['nombre_comun' => 'cropname'])->where(['id' => $id, 'status' => 1])->first();
            $resultj = json_encode($detalle);
            $this->response->type('json');
            $this->response->body($resultj);
            return $this->response;
    ...
    public function provincia($id = null)
    {
        if ($this->request->is('ajax')) {
            $this->autoRender = false;
            $provincias = $this->Ubigeo->find()->select(['cod_pro','nombre'])->where(['cod_dep' => $id, 'cod_pro !=' => 0, 'cod_dis' => 0])->order(['nombre' => 'ASC']);
    ...
    public function distrito($depid = null, $proid = null)
    {
        if ($this->request->is('ajax')) {
            $this->autoRender = false;
            $distritos = $this->Ubigeo->find()->select(['id', 'nombre'])->where(['cod_dep' => $depid, 'cod_pro' => $proid, 'cod_dis !=' => 0])->order(['nombre' => 'ASC']);
   ...
   public function datatablepassportfito()
    {
        if ($this->request->is('ajax')) {
            ...
            if($totalData > 0){
                ...
                $sql = "SELECT @rownum := @rownum + 1 AS item, s.* FROM (
                    SELECT UPPER(a.accenumb) AS accenumb, 
                           UPPER(a.instcode) AS instcode,
                           UPPER(a.othenumb) AS othenumb, 
                           UPPER(a.accname) AS accname,
                           f_datatables_methods(b.subtype) AS subtype, 
                           UPPER(b.collnumb) AS collnumbs,
                           (CONCAT(c.genus,'  ',c.species)) AS especie,
                           c.cropname AS cropname, 
                           UPPER(b.spauthor) AS spauthor, 
                           UPPER(b.collname) AS collname,
                           UPPER(b.subtaxa) AS subtaxa, 
                           UPPER(b.subtauthor) AS subtauthor, 
                           f_datatables_methods(b.storage) AS storage,
                           DATE_FORMAT(b.acqdate,'%d-%m-%Y') AS acqdate,
                           UPPER(d.eea) AS station_current_id,
                           UPPER(e.eea) AS station_origin_id, 
                           f_datatables_methods(b.availability) AS availability,
                           b.id AS passport_fito_id, 
                           UPPER(f.colname) AS colecction,
                           a.station_current_id AS estacion_permiso,
                           b.bfield AS bfield,
                           b.invitro AS invitro,
                           b.bseed AS bseed,
                           f_datatables_methods(a.promissory) AS promissory,
                           c.autor AS autor
                      FROM passport AS a
                ...
                " ORDER BY a.accenumb DESC ) s WHERE 1=1 ".$filtros2;
        ...
        if(!empty($this->request->data['search']['value'])){
            ...
            $sql.=" OR s.promissory LIKE '".$this->request->data['search']['value']."%' )";
        ...
        if( $totalFilter > 0){	
            ...
            $sql.=" ORDER BY ".$colTableName[$this->request->data['order'][0]['column']]." ".$this->request->data['order'][0]['dir']." LIMIT ".
            ...
            while($row = $query->fetch()) {
                ...
                $subdata = array();
                $subdata[] = $row[0];
                $subdata[] = '<a href="'.$this->request->referer().'/ver/'.$row[18].'"  data-toggle="tooltip" title="Ver Detalle"><strong>'.$row[1].'</strong></a>';//COD ACCESION
                $subdata[] = $row[4];//NOMB ACCESION   
                $subdata[] = $row[6];//COD COLEC
                $subdata[] = $row[3];//OTRO COD
                $subdata[] = $row[19];//COLECCION
                $subdata[] = "<i>".$row[7]."</i> ".$row[25];//NOM CIENT
                ...
                $subdata[] = $row[2];//FAO
                $subdata[] = "<center>".$row[24]."</center>";//PRO
                $subdata[] = "<center>".$row[17]."</center>";//DISP
                       
    
                        
        /*B.CAMPO*/    if($row[21]=="39"){$campo="<span class='badge badge-success'>Campo</span>";}else if($row[21]=="40"){$campo="<span class='badge'>Campo</span>";}else{$campo=NULL;}
        /*B.INVITRO*/  if($row[22]=="33"){$invitro="<span class='badge badge-danger'><i>In vitro</i></span>";}else if($row[22]=="34"){$invitro="<span class='badge'><i>In vitro</i></span>";}else{$invitro=NULL;}
        /*B.SEMILLAS*/ if($row[23]=="36"){$semilla="<span class='badge badge-warning'>Semillas</span>";}else if($row[23]=="37"){$semilla="<span class='badge'>Semillas</span>";}else{$semilla=NULL;}
                        
                        $subdata[] =$campo. " " .$invitro. " " .$semilla;
					...
					$header_titles = array('ITEM','COD. ACCESIÓN','NOMBRE ACCESIÓN','CÓDIGO DE COLECTA','OTRO COD. ACCESIÓN','COLECCIÓN','NOMBRE CIENTÍFICO','NOMBRE COMÚN','FECHA ADQUISICIÓN','SUBTIPO RECURSO','TIPO CONSERVACIÓN','EST. EXPERIMENTAL','COD. FAO','DISPONIBILIDAD','PROMISORIA');
