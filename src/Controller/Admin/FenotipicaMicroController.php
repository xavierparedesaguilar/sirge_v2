 ...
 */
class FenotipicaMicroController extends AppController
{
    ...
    public function importar()
    {
        if($this->permiso['import']){
                            ...
                            if($highestRow > 1){
                                ...
                                if($total_column == 3)
                                {
                                    ...
                                    if($cont == $total_column){
                                        ...
                                        $sql_1 = $conn->prepare(" DELETE FROM temp_descriptores WHERE recurso = 3 AND tipo_carga = 1 AND user_id = ? ");
                                        ...
                                        for ($row = 2; $row <= $highestRow; $row++){
                                            ...
                                            $temp_caract->recurso = '3';
                                            $temp_caract->tipo_carga = '1';
                                            $temp_caract->user_id = $user_id;
                                            $temp->save($temp_caract);
                                        }

                                        $stmt = $conn->prepare(' CALL usp_import_descriptor(3, 1, 1, 0, ?, ?, ?) ');
                                        ...
                                        $temp_descriptor = $this->TempDescriptores->find('all')->where(['user_id' => $user_id, 'recurso' => 3, 'tipo_carga' => '1'])->toArray();

                                        $this->Flash->success(__('Descriptores importados exitosamente.'));

                                        $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'temp_descriptor', 'model_specie', 'tipo_agrupacion', 'scripts'));
                            ...
                            if($highestRow > 1){
                                ...
                                if($total_column == 3)
                                {
                                    ...
                                    if($cont == $total_column){
                                         ...
                                         /******** SE ELIMINA LOS REGISTROS ANTERIORES ********/
                                        $sql_1 = $conn->prepare(" DELETE FROM temp_descriptores WHERE recurso = 3 AND tipo_carga = 2 AND user_id = ? ");
                                        $sql_1->bindValue(1, $user_id, PDO::PARAM_STR);
                                        $sql_1->execute();
                                        ...
                                        $stmt = $conn->prepare(' CALL usp_import_descriptor(3, 1, 2, 0, ?, ?, ?) ');
                                        $stmt->bindValue(1, $model_specie->id, PDO::PARAM_STR);
                                        $stmt->bindValue(2, $model_specie->collection_id, PDO::PARAM_STR);
                                        $stmt->bindValue(3, $user_id, PDO::PARAM_STR);
                                        $stmt->execute();

                                        $temp_state = $this->TempDescriptores->find('all')->where(['user_id' => $user_id, 'recurso' => 3, 'tipo_carga' => '2'])->toArray();

                                        $this->Flash->success(__('Estado de descriptores importados exitosamente.'));

                                        $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'temp_state', 'model_specie', 'tipo_agrupacion', 'scripts'));
                                        unlink($inputFileName);
            ...
            $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'scripts','especie_idx','value_specie','especie_nombre','especie_idy','value_specie_idy','especie_nombre_idy'));
    ...
    public function uploadfile()
    {
        if ($this->request->is('post')) {
            ...
            if($data['form_tipo'] == 1){
                ...
                $stmt = $conn->prepare(' CALL usp_import_descriptor( 3, 2, 1, ?, ?, ?, ? ) ');
                ...
                $stmt = $conn->prepare(' CALL usp_import_descriptor( 3, 2, 2, ?, ?, ?, ? ) ');
                $stmt->bindValue(1, $tipo_agrupacion, PDO::PARAM_STR);
                $stmt->bindValue(2, $data['especie_id'], PDO::PARAM_STR);
                $stmt->bindValue(3, $data['colection_id'], PDO::PARAM_STR);
                $stmt->bindValue(4, $uid, PDO::PARAM_STR);
                    ...
                    try
                    {
                        ...
                        $query = $this->Descriptor->find()->select(['Descriptor.name'])->innerJoinWith('Specie')
                                                          ->where(['Descriptor.specie_id' => $data['especie_id'],
                                                                   ...
                                                                   'Descriptor.status' => 1, 'Descriptor.resource_id' => 3,
                                                                   'Descriptor.type' => 1])->group('1');
                        ...
                        if($total_column == $total_columnas)
                        {
                            ...
                            if($cont == $total_columnas){
                                ...
                                {
                                    /******** SE ELIMINA LOS REGISTROS ANTERIORES ********/
                                    $sql_1 = $conn->prepare(" DELETE FROM temp_caracterizacion WHERE resource_id = 3 AND specie_name = ? AND collection_name = ? AND user_id = ? ");
                                    ...
                                    if($contador > 0){
                                        ...
                                        $stmt = $conn->prepare(' CALL usp_import_caracterizacion(3, 1, ?, ?, ?) ');
                                        $stmt->bindValue(1, $data['especie_id'], PDO::PARAM_INT);
                                        $stmt->bindValue(2, $data['coleccion_id'], PDO::PARAM_INT);
                                        $stmt->bindValue(3, $user_id, PDO::PARAM_INT);
                                        $stmt->execute();

                                        $stmt_1 = $conn->prepare(' CALL usp_import_caracterizacion_table( ?, ?, 3, ? ) ');
        ...
        if($this->permiso['export']){
            ...
            if ($this->request->is('post')) {
                ...
                $leyenda = $this->Descriptor->find()->select(['name' => 'Descriptor.name', 'descripcion' => 'Descriptor.title', 'code' => 'DescriptorState.code', 'label' => 'DescriptorState.label'])
                                                        ...
                                                        ->where(['Descriptor.resource_id' => '3', 'Descriptor.type' => '1', 'Descriptor.specie_id' => $data['especie_id'],
                ...
                $query = " CALL usp_formato_caracterizacion( ? , ? , 3 ) ";
                ...
                if(!empty($resultado))
                {
                    ...
                    $objPHPExcel->getActiveSheet()->setTitle('MICROORGANISMO-CARACTERIZACIÓN');
                    ...
                    $filename = "micro_feno_".$especie['cropname'].".xlsx";
    ...
    public function cargarCaracterizacion()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $stmt = $conn->prepare(' CALL usp_import_caracterizacion( 3, 2, ?, ?, ? ) ');
            $stmt->bindValue(1, $data['especie_id'], PDO::PARAM_INT);
            $stmt->bindValue(2, $data['coleccion_id'], PDO::PARAM_INT);
            $stmt->bindValue(3, $user_id, PDO::PARAM_INT);
            $stmt->execute();

            $this->Flash->success(__('Importación de Fenotípica Microorganismo fue realizado satisfactoriamente.'));
            return $this->redirect(['controller' => 'DescriptorMicro', 'action' => 'caracterizacion', 'idx' => $data['especie_id'], 'idy' => $data['coleccion_id'] ]);
        }
