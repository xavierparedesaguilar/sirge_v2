 ...
 */
class ExtractionDnaMicroController extends AppController
{
    ...
    public function index($id=null)
    {
         ...
         return $this->redirect(['controller' => 'BankDnaMicro', 'action' => 'index']);
    }
    ...
    {
        $extractionDna_count = $this->BankDna->find()->where(['BankDna.status '=>'1','BankDna.id'=>$id,'BankDna.type_resource'=>3])->count();
    ...
    public function edit($id = null)
    {
        ...
        if($bankDnaMicro_count>0 && $this->permiso['edit']){

            $extractionDna = $this->ExtractionDna->find()
                                         ->where(['ExtractionDna.status !=' => '0','ExtractionDna.bank_dna_id'=>$id
                                            ])->first();
    ...
    public function delete($id = null,$child=null)
    {
        ...
        if($bankDnaMicro_count>0 && $this->permiso['delete']){

            $this->request->is(['post', 'delete']);

            $extracionDnaMicro = $this->ExtractionDna->find()
                                         ->where(['ExtractionDna.status !=' => '0','ExtractionDna.id'=>$child,'ExtractionDna.bank_dna_id'=>$id
                                            ])->first();

            if($extracionDnaMicro==NULL){

                //$this->Flash->error(__('Operación denegada.'));
                return $this->redirect(['action' => 'index',$id]);
            }else{

                $extracionDnaMicro['status'] = 0;

                if ($this->InputDna->save($extracionDnaMicro)) {

                    $list_module = explode('/', $this->request->params['_matchedRoute']);
                    $user_id    = $this->Auth->User('id');
                    $module     = $list_module[(count($list_module)-4)];
                    $action     = $list_module[(count($list_module)-2)].'/'.$list_module[(count($list_module)-1)];
                    $station_id = $extracionDnaMicro->id;
                    $recurso_id = '3';
               ...
               return $this->redirect(['action' => 'index', $extracionDnaMicro->bank_dna_id]);
            }
