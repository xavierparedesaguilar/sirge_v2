 ...
 */
class FenotipicaFitoController extends AppController
{
    ...
    public function index()
    {
        ...
        if($this->permiso['index']){
            ...
            if ($this->request->is('post')) {
                ...
                $this->set(compact('mod_padre', 'mod_parent', 'colecciones','permiso', 'scripts'));
            }
                                ...
                                if($highestRow > 1){
                                    ...
                                    if($total_column == 3)
                                    {
                                        ...
                                        if($cont == $total_column){
                                            ...
                                            $sql_1 = $conn->prepare(" DELETE FROM temp_descriptores WHERE recurso = 1 AND tipo_carga = 1 AND user_id = ? ");
                                            ...
                                            for ($row = 2; $row <= $highestRow; $row++){
                                                ...
                                                $temp_caract->recurso = '1';
                                                $temp_caract->tipo_carga = '1';
                                                $temp_caract->user_id = $user_id;
                                                $temp->save($temp_caract);
                                            }

                                            $stmt = $conn->prepare(' CALL usp_import_descriptor(1, 1, 1, 0, ?, ?, ?) ');
                                            ...
                                            $temp_descriptor = $this->TempDescriptores->find('all')->where(['user_id' => $user_id, 'recurso' => 1, 'tipo_carga' => '1'])->toArray();

                                            $this->Flash->success(__('Descriptores importados exitosamente.'));

                                            $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'temp_descriptor', 'model_specie', 'tipo_agrupacion', 'scripts','permiso','especie_idx','value_specie','especie_nombre'));
                                ...
                                if($highestRow > 1){
                                    ...
                                    if($total_column == 3)
                                    {
                                        ...
                                        if($cont == $total_column){
                                            ...
                                            /******** SE ELIMINA LOS REGISTROS ANTERIORES ********/
                                            $sql_1 = $conn->prepare(" DELETE FROM temp_descriptores WHERE recurso = 1 AND tipo_carga = 2 AND user_id = ? ");
                                            $sql_1->bindValue(1, $user_id, PDO::PARAM_STR);
                                            $sql_1->execute();
                                            ...
                                            $stmt = $conn->prepare(' CALL usp_import_descriptor(1, 1, 2, 0, ?, ?, ?) ');
                                            $stmt->bindValue(1, $model_specie->id, PDO::PARAM_STR);
                                            $stmt->bindValue(2, $model_specie->collection_id, PDO::PARAM_STR);
                                            $stmt->bindValue(3, $user_id, PDO::PARAM_STR);
                                            $stmt->execute();

                                            $temp_state = $this->TempDescriptores->find('all')->where(['user_id' => $user_id, 'recurso' => 1, 'tipo_carga' => '2'])->toArray();

                                            $this->Flash->success(__('Estado de descriptores importados exitosamente.'));

                                            $this->set(compact('descriptor', 'mod_padre', 'mod_parent', 'colecciones', 'temp_state', 'model_specie', 'tipo_agrupacion', 'scripts','permiso','especie_idy','value_specie_idy','especie_nombre_idy'));
                                            unlink($inputFileName);
    ...
    public function uploadfile()
    {
        if ($this->request->is('post')) {
            ...
            if($data['form_tipo'] == 1){
                ...
                $stmt = $conn->prepare(' CALL usp_import_descriptor( 1, 2, 1, ?, ?, ?, ? ) ');
                ...
                $stmt = $conn->prepare(' CALL usp_import_descriptor( 1, 2, 2, ?, ?, ?, ? ) ');
                $stmt->bindValue(1, $tipo_agrupacion, PDO::PARAM_STR);
                $stmt->bindValue(2, $data['especie_id'], PDO::PARAM_STR);
                $stmt->bindValue(3, $data['colection_id'], PDO::PARAM_STR);
                $stmt->bindValue(4, $uid, PDO::PARAM_STR);
                        ...
                        try
                        {
                            ...
                            $query = $this->Descriptor->find()->select(['Descriptor.name'])->innerJoinWith('Specie')
                                                              ->where(['Descriptor.specie_id' => $data['especie_id'],
                                                                       ...
                                                                       'Descriptor.status' => 1, 'Descriptor.resource_id' => 1])->group('1');
                            ...
                            if($total_column == $total_columnas)
                            {
                                ...
                                array_unshift($columnas_,'CÓDIGO ACCESIÓN', 'NOMBRE ACCESIÓN', 'AÑO DE CAMPAÑA');
                                ...
                                if($cont == $total_columnas){
                                    ...
                                    {
                                        /******** SE ELIMINA LOS REGISTROS ANTERIORES ********/
                                        $sql_1 = $conn->prepare(" DELETE FROM temp_caracterizacion WHERE resource_id = 1 AND specie_name = ? AND collection_name = ? AND user_id = ? ");
                                        ...
                                        if($contador > 0){
                                            ...
                                            $stmt = $conn->prepare(' CALL usp_import_caracterizacion(1, 1, ?, ?, ?) ');
                                            ...
                                            /************ EJECUTA PARA MOSTRAR EN PANTALLA LOS RESULTADOS **********/
                                            $stmt_1 = $conn->prepare(' CALL usp_import_caracterizacion_table( ?, ?, 1, ? ) ');
        ...
        if($this->permiso['export']){
                ...
                if ($this->request->is('post')) {
                    ...
                    $leyenda = $this->Descriptor->find()->select(['name' => 'Descriptor.name', 'descripcion' => 'Descriptor.title', 'code' => 'DescriptorState.code', 'label' => 'DescriptorState.label'])
                                                            ...
                                                            ->where(['Descriptor.resource_id' => '1', 'Descriptor.specie_id' => $data['especie_id'],
                    ...
                    if(!empty($resultado))
                    {
                        ...
                        $objPHPExcel->getActiveSheet(0)->getStyle('A1:'.$letra[$t-2].'1')->applyFromArray($estiloTabla);
                        ...
                        $objPHPExcel->getActiveSheet()->setTitle('FITOGENÉTICO - CARACTERIZACIÓN');
                        ...
                        $this->Flash->error(__('No existen descriptores y/o estados para la colección y especie seleccionados.'));
                        return $this->redirect(['action'=> 'importarCaracterizacion']);
                    }
                }
    ...
    public function cargarCaracterizacion()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $stmt = $conn->prepare(' CALL usp_import_caracterizacion( 1, 2, ?, ?, ? ) ');
            $stmt->bindValue(1, $data['especie_id'], PDO::PARAM_INT);
            $stmt->bindValue(2, $data['coleccion_id'], PDO::PARAM_INT);
            $stmt->bindValue(3, $user_id, PDO::PARAM_INT);
            $stmt->execute();
            ...
            return $this->redirect(['controller' => 'DescriptorFito', 'action' => 'caracterizacion', 'idx' => $data['especie_id'], 'idy' => $data['coleccion_id'] ]);
