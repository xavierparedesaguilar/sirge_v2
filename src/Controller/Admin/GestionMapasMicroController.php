 ...
 */
class GestionMapasMicroController extends AppController
{
    ...
    public function index()
    {
        ...
        if($this->permiso['index']){
            ...
            $collection = $this->Collection->find('list', ['keyField' => 'id', 'valueField' => 'colname'])
                                           ->where(['status' => '1', 'resource_id' => 3, 'availability' => '1'])->order(['colname' => 'ASC']);
    ...
    public function buscar( $id = NULL )
    {
        ...
        if(substr($id, 0,1) == '1'){
            ...
            $passport = $this->PassportMicro->find()->contain('Passport.Specie')->innerJoinWith('Passport.Specie')
                             ->select(['id'        => 'Passport.id',
                                       ...
                                       'latitude'  => 'PassportMicro.latitude',
                                       'longitude' => 'PassportMicro.longitude'])
                             ->where(['Passport.status' => '1', 'Passport.specie_id' => $model_especie->id,
                                      'Specie.collection_id' => $model_especie->collection_id, 'Passport.resource_id' => '3'])->order(['Passport.accname' => 'DESC'])->all();
            ...
            $passport = $this->PassportMicro->find()->contain('Passport.Specie')->innerJoinWith('Passport.Specie')
                             ...
                             ->where(['Passport.status' => '1', 'Specie.collection_id' => $model_coleccion->id, 
									  'Passport.resource_id' => '3'])->order(['Passport.accenumb' => 'ASC'])->all();
    ...
    public function ver()
    {
        ...
        if ($this->request->is('post')) {
			...
			foreach ($porciones as $a){
				$model = $this->PassportMicro->find()->contain(['Passport.Specie'])->innerJoinWith('Passport.Specie')
                              ...
                              ->where([ 'PassportMicro.passport_id' => $a ])->first();
            ...
            /*foreach ($data['passport'] as $key => $value) {
                ...
                $model = $this->PassportMicro->find()->contain(['Passport.Specie'])
                              ->select(['accenumb' => 'Passport.accenumb',
                                        'latitude' => 'PassportMicro.latitude',
                                        'longitude' => 'PassportMicro.longitude',
                                        'id' => 'PassportMicro.id',
                                        'genero' => 'UPPER(Specie.genus)', 'especie' => 'UPPER(Specie.species)'])
                              ->where([ 'PassportMicro.passport_id' => $value['passport_id'] ])->first();
            ...
            for ($i=0; $i < count($list_passport); $i++) {
                ...
                $shp->records[$i]->DBFData['RECURSO']  = 'MICROORGANISMO';
            ...
            $shp->saveToFile(WWW_ROOT.'pass_microorganismo'.DS.'gestion_mapas'.DS.'shapefile/shp_micro.*');

            ///******************  COMPRIMIR EN FORMATO ZIP LOS ARCHIVOS CREADOS  *******************//
            $zip = new \ZipArchive();

            $user_id = $this->Auth->User('id');
            $filename = WWW_ROOT.'pass_microorganismo'.DS.'gestion_mapas'.DS.'shapefile/shp_micro_'.$user_id.'.zip';
            $directorio = 'pass_microorganismo'.DS.'gestion_mapas'.DS.'shapefile'.DS;

            if($zip->open($filename, \ZIPARCHIVE::CREATE)===true) {
                    $zip->addFile($directorio.'shp_micro.dbf');
                    $zip->addFile($directorio.'shp_micro.shp');
                    $zip->addFile($directorio.'shp_micro.shx');
                    $zip->close();
            }
    ...
    public function importarImagen()
    {
        if ($this->request->is('post')) {
            ...
            if(isset($_POST['boton_1'])){
                ...
                $src = $data['url_mapa'];
                $desFolder = 'pass_microorganismo/gestion_mapas/';
                $imageName = 'google_map.jpg';
                $imagePath = $desFolder.$imageName;
                file_put_contents($imagePath,file_get_contents($src));

                $filePath = WWW_ROOT .'pass_microorganismo'. DS . 'gestion_mapas'. DS .'google_map.jpg';
            ...
            if(isset($_POST['boton_2'])){
                ...
                $filePath = WWW_ROOT .'pass_microorganismo'. DS . 'gestion_mapas'. DS .'google_map.tiff';
            ...
            if(isset($_POST['boton_3'])){
                ...
                $filePath = WWW_ROOT .'pass_microorganismo'. DS . 'gestion_mapas'. DS .'google_map.png';
            ...
            if(isset($_POST['boton_4'])){
                ...
                $filePath = WWW_ROOT.'pass_microorganismo'.DS.'gestion_mapas'.DS.'shapefile'.DS.'shp_micro_'.$user_id.'.zip';
