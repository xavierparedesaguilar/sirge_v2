 ...
 */
class DescriptorZooController extends AppController
{
    public function initialize()
    {
        ...
        $this->module = $this->Module->find()->where(['controller' => 'FenotipicaZoo'])->first();
    ...
    public function index( $id = null )
    {
        if($this->permiso['index']){
            ...
            $especie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $id,'Collection.resource_id'=>2])->first();
            if ($especie!=null) {
            ...
            $descriptor = $this->Descriptor->find()->contain(['Specie','Resource'])
                               ->where(['Descriptor.specie_id' => $id, 'Descriptor.resource_id' => '2', 'Specie.cropname' => $especie->cropname, 'Descriptor.status !=' => '0'])->all();
            ...
            return $this->redirect(['action' => 'index','controller'=>'FenotipicaZoo']);
    ...
    {
        $especie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $idx,'Collection.resource_id'=>2])->first();

        if ($especie!=NULL && $this->permiso['view']) {

            $descriptor = $this->Descriptor->find()->contain('Specie')->where(['Descriptor.id' => $id,'Descriptor.status' =>1,'Descriptor.resource_id'=>2])->first();
            ...
            $this->set('_serialize', ['descriptor']);
                ...
                $this->Flash->error(__('Operación denegada.'));
                return $this->redirect(['action' => 'index','controller'=>'DescriptorZoo',$idx]);
            }
    ...
    public function edit($idx = null, $id = null)
    {
        ...
        if ($especie!=NULL && $this->permiso['edit']) {
            ...
            $descriptor = $this->Descriptor->find()->where(['Descriptor.status '=>'1','id' => $id,'Descriptor.resource_id'=>2,'Descriptor.specie_id' => $idx])->first();
    ...
    public function caracterizacion($idx = null, $idy = null)
    {
        ...
        $mod_specie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $idx,'Collection.resource_id'=>2,'Collection.id'=>$idy])->first();
        ...
        if($mod_specie!=NULL) {
            ...
            $stmt = $conn->prepare(' CALL usp_caracterizacion_table( ?, ?, 2 ) ');
    ...
    public function ordenar($idx = null, $idy = null)
    {
        ...
        $especie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $idx,'Collection.resource_id'=>2,'Collection.id'=>$idy])->count();
        ...
        if($especie>0) {
            ...
            $mod_specie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $idx,'Collection.resource_id'=>2,'Collection.id'=>$idy])->first();
                ...
                $descriptor_especie = $this->Descriptor->find()->contain('Specie.Collection')->where(['Descriptor.resource_id' => 2, 'Descriptor.status' => '1' ,'Specie.id' => $idx,'Collection.id' => $idy])->order(['position' => 'ASC'])->all();
