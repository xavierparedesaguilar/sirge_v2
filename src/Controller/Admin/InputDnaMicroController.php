 ...
 */
class InputDnaMicroController extends AppController
{
    ...
    public function view($id = null,$child=null)
    {
        ...
        if($bankDnaMicro_count ==NULL || !$this->permiso['view']){
            ...
            $this->set(compact('inputDna','titulo','permiso','id','validar'));
            $this->set('_serialize', ['inputDna']);
        }
    }
    ...
    {
        $bankDnaMicro_count = $this->BankDna->find()->where(['BankDna.status '=>'1','BankDna.id'=>$id,'BankDna.type_resource'=>3])->count();

        if($bankDnaMicro_count>0 && $this->permiso['add']){
    ...
    public function edit($id = null,$child=null)
    {
       ...
       if($bankDnaMicro_count!=NULL && $this->permiso['edit']  /*&& $validar*/ ){
        ...
        $this->set(compact('inputDna','modulo','lista_deposito','lista_muestra','lista_estado','scripts','id'));
        $this->set('_serialize', ['inputDna']);
    ...
    public function delete($id = null,$child=null)
    {
        ...
        if($bankDnaMicro_count!=NULL && $this->permiso['delete'] /*&& $validar*/){
            ...
            $inputDnaMicro = $this->InputDna->find()
                                         ->where(['InputDna.status !=' => '0','InputDna.id'=>$child,'InputDna.bank_dna_id'=>$id
                                            ])->first();

            if($inputDnaMicro==NULL){

                $this->Flash->error(__('Operación denegada.'));
                return $this->redirect(['action' => 'index',$id]);
            }else{

                $inputDnaMicro['status'] = 0;

                if ($this->InputDna->save($inputDnaMicro)) {
               ...
               return $this->redirect(['action' => 'index', $inputDnaMicro->bank_dna_id]);
            }
