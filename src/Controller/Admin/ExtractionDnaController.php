...
class ExtractionDnaController extends AppController
{
    ...
    public function index($id=null)
    {
         ...
         return $this->redirect(['controller' => 'BankDna', 'action' => 'index']);
    }
    ...
    {
         $extractionDna_count = $this->BankDna->find()->where(['BankDna.status '=>'1','BankDna.id'=>$id,'BankDna.type_resource'=>1])->count();
    ...
    public function delete($id = null,$child=null)
    {
        ...
        if($bankDna_count>0 && $this->permiso['delete']){
            ...
            $extracionDna = $this->ExtractionDna->find()
                                         ->where(['ExtractionDna.status !=' => '0','ExtractionDna.id'=>$child,'ExtractionDna.bank_dna_id'=>$id
                                            ])->first();

            if($extracionDna==NULL){

                //$this->Flash->error(__('Operación denegada.'));
                return $this->redirect(['action' => 'index',$id]);
            }else{

                $extracionDna['status'] = 0;

                if ($this->InputDna->save($extracionDna)) {
               ...
               return $this->redirect(['action' => 'index', $extracionDna->bank_dna_id]);
            }
