 ...
 */
class CaractGenotypicZooController extends AppController
{
    ...
    public function index()
    {
        if($this->permiso['index']){
            ...
            $caractGenotypic = $this->CaractGenotypic->find()->where(['status' => '1', 'resource_id' => 2])->all();
    ...
    public function view($id = null)
    {
        if($this->permiso['view']){
            ...
            $caractGenotypic = $this->CaractGenotypic->find()->where(['CaractGenotypic.status' => '1','CaractGenotypic.id' => $id,'CaractGenotypic.resource_id '=>2])->first();
    ...
    public function add()
    {
        if($this->permiso['add']){
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                $data['CaractGenotypic']['resource_id'] = '2';
                ...
                /********* ARCHIVO 1 *********/
                $data['file_1']['name'] =  'accenumb_'.$caractGenotypic->numeroDocumentoZoo.$extension;
                $fichero_subido_1 = $dir_subida . basename($data['file_1']['name']);
                if(move_uploaded_file($data['file_1']['tmp_name'], $fichero_subido_1))
                    $data['CaractGenotypic']['accenumb'] = 'pass_zoogenetico/genotipica/'. 'accenumb_'.$caractGenotypic->numeroDocumentoZoo.$extension;

                /********* ARCHIVO 2 *********/
                $data['file_2']['name'] =  'datamatrix_'.$caractGenotypic->numeroDocumentoZoo.$extension_;
                $fichero_subido_2 = $dir_subida . basename($data['file_2']['name']);
                if(move_uploaded_file($data['file_2']['tmp_name'], $fichero_subido_2))
                    $data['CaractGenotypic']['datamatrix'] = 'pass_zoogenetico/genotipica/'. 'datamatrix_'.$caractGenotypic->numeroDocumentoZoo.$extension_;
                ...
                $this->Flash->error(__('Hubo inconvenientes al crear el Pasaporte Zoogenético. Por favor, Otra vez intente.'));
            }
            ...
            $coleccion         = $this->Collection->find('list', ['keyField' => 'id', 'valueField' => 'colname'])->where(['resource_id' => 2, 'status' => 1, 'availability' => '1']);
            $marcador_molecular= $this->OptionList->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['status' => 1, 'parent_id' => 421, 'OR' => [['resource_id' => 2], ['resource_id' => 4]] ]);
            $localizacion      = $this->OptionList->find('list', ['keyField' => 'id', 'valueField' => 'name'])->where(['status' => 1, 'parent_id' => 422, 'OR' => [['resource_id' => 2], ['resource_id' => 4]] ]);
    ...
    {
        $caractGenotypic = $this->CaractGenotypic->find()->where(['CaractGenotypic.id' => $id])->first();

        if ($caractGenotypic!=NULL && $this->permiso['edit']) {
            ...
            if ($this->request->is(['patch', 'post', 'put'])) {
                ...
                $fichero_subido_1 = $dir_subida . basename($data['file_1']['name']);
                if(move_uploaded_file($data['file_1']['tmp_name'], $fichero_subido_1))
                    $data['CaractGenotypic']['accenumb'] = 'pass_zoogenetico/genotipica/'. 'accenumb_'.$caractGenotypic->expnumb.$extension;

                /********* ARCHIVO 2 *********/
                $data['file_2']['name'] =  'datamatrix_'.$caractGenotypic->expnumb.$extension_;
                $fichero_subido_2 = $dir_subida . basename($data['file_2']['name']);
                if(move_uploaded_file($data['file_2']['tmp_name'], $fichero_subido_2))
                    $data['CaractGenotypic']['datamatrix'] = 'pass_zoogenetico/genotipica/'. 'datamatrix_'.$caractGenotypic->expnumb.$extension_;
                ...
                $this->Flash->error(__('Hubo inconvenientes al actualizar el Pasaporte Zoogenético. Por favor, Otra vez intente.'));
            }
            ...
            $this->Flash->error(__('Operación denegada.'));
            return $this->redirect(['action' => 'index','controller'=>'CaractGenotypicZoo']);
    ...
    public function delete($id = null)
    {
        ...
        if ($caractGenotypic!=NULL && $this->permiso['delete']) {
