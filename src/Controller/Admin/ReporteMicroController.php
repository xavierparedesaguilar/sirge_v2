 ...
 */
class ReporteMicroController extends AppController
{
    ...
    public function initialize()
    {
        ...
        $this->mod_parent = "";
        $this->mod_padre = "Módulo de Reportes Microorganismo";
    ...
    public function getDataReporte($tipoReporte = null)
    {
        ...
        if($tipoReporte==631){
            ...
            $sql="SELECT stacion,SUM(total) total FROM
                          ...
                          (SELECT DISTINCT sta.eea stacion ,pass.othenumb acesion
                              ...
                              INNER JOIN passport_micro micro ON micro.passport_id=pass.id AND micro.availability=27
        ...
        if($tipoReporte==636){
            ...
            $sql="
                  ...
                  SELECT 'Micro', COUNT(DISTINCT descripcion) total  FROM
                      (SELECT  pass.id ,pass.othenumb descripcion
                      FROM passport pass
                        INNER JOIN passport_micro micro ON micro.passport_id=pass.id AND micro.availability=27
                        WHERE   pass.id IN ( SELECT passport_id FROM bank_micro WHERE status=1 )
                      AND status=1 AND pass.resource_id=$this->tipo_recurso AND pass.othenumb<>'')
                          TABLA" ;
