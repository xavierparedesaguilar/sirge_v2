 ...
 */
class DescriptorStateZooController extends AppController
{
    ...
    {   //$idx=fenotipica $idy= descriptor
        $especie = $this->Specie->find()->where(['Specie.id' => $idx])->first();

        if ($especie!=NULL && $this->permiso['index']) {

            $descriptor = $this->Descriptor->find()->where(['Descriptor.status '=>'1','id' => $idy,'Descriptor.resource_id'=>2,'Descriptor.specie_id'=>$idx])->first();
            ...
            if($descriptor!=NULL){
                ...
                $descriptorState = $this->DescriptorState->find()->contain('Descriptor')->where(['DescriptorState.descriptor_id' => $idy, 'DescriptorState.status !=' => '0','Descriptor.status' => 1, 'Descriptor.resource_id' =>'2'])->order('DescriptorState.code ASC')->all();
     ...
     */
    public function view($idx,$idy,$id = null)
    {

        if($this->permiso['view']){
            $this->Flash->error(__('Operación denegada.'));
            return $this->redirect(['action' => 'index','controller'=>'DescriptorStateZoo',$idx,$idy]);

            // $permiso = $this->permiso;


            // $descriptorState = $this->DescriptorState->find()->contain('Descriptor')
            //                                          ->where(['DescriptorState.status !=' => '0'])->first();

            // $this->set(compact('descriptorState', $descriptorState,'permiso'));
            // $this->set('_serialize', ['descriptorState']);
    ...
    public function add( $idx = null, $idy = null )
    {
         ...
         $descriptor = $this->Descriptor->find()->where(['id' => $idy,'status' => 1, 'resource_id' => 2 ,'specie_id'=>$idx])->first();
    ...
    { //idy=desriptor,idx=especie,$id= estadodescriptor
        ...
        if ($especie!=NULL && $this->permiso['edit']) {
            ...
            if($descriptor!=NULL){
                ...
                //$this->Flash->error(__('Operación denegada22.'));
                return $this->redirect(['action' => 'index','controller'=>'DescriptorStateZoo',$idx,$idy]);
