 ...
 */
class DescriptorBioquimicaController extends AppController
{
    public function initialize()
    {
        parent::initialize(); // TODO: Change the autogenerated stub
        $this->loadComponent('Csrf');
        $this->mod_padre = "Bioquímica";
        ...
        if($this->permiso['index']){
            ...
            if($especie!=NULL){
                ...
                $descriptor = $this->Descriptor->find()->contain(['Specie', 'Resource'])
                                   ->where(['Descriptor.specie_id' => $id, 'Descriptor.type' => 2, 'Descriptor.resource_id' => '3', 'Specie.cropname' => $especie->cropname, 'Descriptor.status !=' => '0'])->all();
                ...
                $this->set('_serialize', ['descriptor']);
                  ...
                  return $this->redirect(['action' => 'index','controller'=>'BioquimicaMicro']);
        ...
        if($this->permiso['add']){
                ...
                if($especie!=NULL) {
                        ...
                        if ($this->request->is('post')) {
                            ...
                            $posicion = $this->Descriptor->find()->innerJoinWith('Specie')->where(['Descriptor.specie_id' => $especie->id, 'Descriptor.type' => 2, 'Specie.collection_id' => $especie->collection_id])
    ...
    public function caracterizacion($idx = null, $idy = null)
    {
        ...
        $mod_specie = $this->Specie->find()->contain('Collection')->where(['Specie.id' => $idx,'Collection.resource_id'=>3,'Collection.id'=>$idy])->first();
        ...
        if($mod_specie>NULL) {
                ...
                $stmt = $conn->prepare(' CALL usp_caracterizacion_table( ?, ?, 4 ) ');
    ...
    public function ordenar($idx = null, $idy = null)
    {
        ...
        if ($this->permiso['edit']) {
                ...
                if($especie>0) {
                    ...
                    $descriptor_especie = $this->Descriptor->find()->contain('Specie.Collection')->where(['Specie.id' => $idx,'Collection.id' => $idy])->order(['position' => 'ASC'])->all();
